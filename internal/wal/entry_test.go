...
var _ = Describe("Entry", func() {
	It("should write an entry", func() {
		...
		Expect(wal.WriteEntry(&buffer, []byte("foo"))).To(Succeed())
	...
	It("should read an entry", func() {
		...
		data, err := wal.ReadEntry(&buffer, nil, int64(buffer.Len()))
...
func BenchmarkEntry_Write(b *testing.B) {
	for i := range []int{0, 1, 2, 4, 8} {
		b.Run(fmt.Sprintf("%d KB data", i), func(b *testing.B) {
			...
			for range b.N {
				if err := wal.WriteEntry(buffer, data); err != nil {
					b.Fatal(err)
				}
			}
		})
...
func BenchmarkEntry_Read(b *testing.B) {
	for i := range []int{0, 1, 2, 4, 8} {
		b.Run(fmt.Sprintf("%d KB data", i), func(b *testing.B) {
			...
			for range b.N {
				if _, err := wal.ReadEntry(buffer, data, int64(buffer.Len())); err != nil {
					b.Fatal(err)
				}
			}
		})
